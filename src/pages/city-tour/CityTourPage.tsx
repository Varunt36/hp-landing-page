...
import {
  ...
  FormControlLabel, FormHelperText, InputLabel, MenuItem, Paper,
...
const DONATION_EUR = Number.isFinite(RAW_DONATION) && RAW_DONATION > 0 ? RAW_DONATION : 30
...
export default function CityTourPage() {
  ...
  const validate = (): Errors => {
    ...
    names.forEach((raw, i) => {
      ...
      else if (n.length > 100)   nameErrors[i] = 'Full name must be 100 characters or fewer.'
      else if (/[<>&]/.test(n))  nameErrors[i] = 'Please remove the characters < > &'
    })
  ...
  return (
    <Box sx={s.outer}>
      <Paper elevation={3} sx={s.paper}>
          ...
          <Box>
            <Box sx={s.phoneRow}>
              ...
              <TextField
                ...
                label="Mobile number"
                value={phone}
                onChange={(e) => { setPhone(e.target.value); setErrors((x) => ({ ...x, phone: undefined })) }}
                error={!!errors.phone}
          ...
          <FormControl error={!!errors.terms}>
            <FormControlLabel
              control={
                <Checkbox
                  checked={terms}
                  onChange={(e) => { setTerms(e.target.checked); setErrors((x) => ({ ...x, terms: undefined })) }}
                  sx={{ color: C.purple600, '&.Mui-checked': { color: C.purple700 } }}
                />
              }
              label={
        ...
        >
          {submitting
            ? <><CircularProgress size={18} sx={{ color: 'white', mr: 1.5 }} />Redirecting…</>
            : 'Continue to payment →'}
        </Button>
